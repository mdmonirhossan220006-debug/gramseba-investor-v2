...
export default async function handler(req){
 ...
 try{
  if(req.method==='GET'&&path==='health') return json({ok:true,service:'gramseba-vercel'});
  ...
  if(req.method==='POST'&&path==='investor/login'){
   ...
   if(!profile||password!==(profile.password||investorEnv().password)||profile.status!=='active'){
    const logs=await getJSON('investor_login_logs',[]);logs.unshift({email:String(email||''),success:false,at:new Date().toISOString()});await setJSON('investor_login_logs',logs.slice(0,200));return json({error:'Investor email বা password ভুল।'},401)
   }
   const logs=await getJSON('investor_login_logs',[]);logs.unshift({email:profile.email,success:true,at:new Date().toISOString()});await setJSON('investor_login_logs',logs.slice(0,200));return json({token:await signRole('investor',profile.id)})
  }
